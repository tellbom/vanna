...
class EnhancedVanna(Qdrant_VectorStore, OpenAI_Chat):
    def __init__(self, client=None, config=None):
        ...
        # 创建自定义的嵌入函数
        embedding_function = LocalSentenceTransformerEmbeddingFunction(model_name)
        ...
        qdrant_config = {
            ...
            # 不使用fastembed嵌入模型，而是使用自定义嵌入函数
            "embedding_function": embedding_function
        }

        # 可以省略embedding_function，因为Qdrant_VectorStore有内置的generate_embedding方法
        # 使用fastembed作为嵌入模型
        ...
        def get_retrieval_context(user: any):
            try:
                ...
                response = {
                    ...
                    "dialect": getattr(self.vn, "dialect", "SQL"),
